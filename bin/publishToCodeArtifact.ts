...
export async function publishToCodeArtifact() {
  const { id, region, profile } = getProdConfig().account;
  ...
  const PACKAGE_DIR = resolve(__dirname, '..', 'packages');
  ...
  const packageDirectories = await getDirectoriesAtPath(PACKAGE_DIR);
  for (const packageDirectory of packageDirectories) {
    modulesList.push(...(await listPrivateModules(packageDirectory)));
  }
  ...
  /**
   *
   * update package-lock.json with CodeArtifact location so SHA matches new location
   * otherwise npm will throw an error when attempting to install from CodeArtifact
   *
   */
  console.log(`>>> updating package-lock file to use 
>>> published artifacts
>>>`);
  await useCodeArtifact({
    account: id,
    region,
    authToken,
